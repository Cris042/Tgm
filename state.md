...
| 2026-07-04 | Descoberta ganhou Bloco I — observabilidade e resiliência (etapa SRE: ferramentas, métricas/SLOs, logs, alertas, saga/compensação/retry/DLQ/rollback) — §6.15.2 atualizado. `README.md` criado (mapa de agentes/skills + passo a passo). |
| 2026-07-04 | Portabilidade: hooks de sessão Obsidian movidos para o repo (`.claude/hooks/` + `.claude/settings.json`, caminho via `$CLAUDE_PROJECT_DIR`) e 10 skills de apoio vendorizadas do nível de usuário — repo autossuficiente ao clonar (roles.md §4.2 e §6.13.5 atualizados). Plugin `ui-ux-pro-max` declarado no settings do projeto. |
